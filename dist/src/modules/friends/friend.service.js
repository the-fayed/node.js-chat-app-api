...
class FriendService {
    ...
    async cancelFriendRequest(data) {
        ...
        const result = await user_model_1.UserModel.bulkWrite(operations, {});
        if (result.modifiedCount < 0) {
            throw new api_error_1.default("Error while canceling friend request!", 400);
        }
    ...
    async getAllFriends(userId) {
        ...
        if (!friends || !friends.length) {
            throw new api_error_1.default("User not found!", 204);
        }
        return friends;
    }
    ...
    async deleteFriend(data) {
        ...
        const result = await user_model_1.UserModel.bulkWrite(operations, {});
        if (result.modifiedCount < 0) {
            throw new api_error_1.default("Error while deleting friend!", 400);
        }
